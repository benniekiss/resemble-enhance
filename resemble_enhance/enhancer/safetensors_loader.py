...
def load_enhancer_from_safetensors(
...
def load_enhancer_from_deepspeed(
    run_dir: Path, device: str = "cpu"
) -> EnhancerInference:
    ...
    filtered_state_dict = {
        k: v for k, v in state_dict.items() if not k.startswith("denoiser.")
    }

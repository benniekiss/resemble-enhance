...
def load_denoiser_from_safetensors(
    model_dir: Union[str, Path], device: str = "cpu"
) -> Denoiser:
...
def load_denoiser_model(
    run_dir: Union[str, Path, None], device: str = "cpu"
) -> Denoiser:
...
def load_denoiser_from_deepspeed(run_dir: Path, device: str = "cpu") -> Denoiser:
    ...
    if not ckpt_path.exists():
        logger.warning(
            f"Model checkpoint not found at {ckpt_path}, returning default model"
        )
        return create_default_denoiser(device)
...
def load_denoiser_from_enhancer_checkpoint(
    run_dir: Union[str, Path, None], device: str = "cpu"
) -> Denoiser:
    ...
    if (
        denoiser_safetensors_dir
        and (denoiser_safetensors_dir / "model.safetensors").exists()
        and (denoiser_safetensors_dir / "config.json").exists()
    ):
    ...
    if denoiser_hp_path.exists():
        ...
        if enhancer_hp_path.exists():
            ...
            if (
                hasattr(enhancer_hp, "denoiser_run_dir")
                and enhancer_hp.denoiser_run_dir
            ):
    ...
    if not denoiser_state_dict:
        ...
        logger.warning(
            "No denoiser weights found in enhancer checkpoint, using default model"
        )
        return create_default_denoiser(device)
